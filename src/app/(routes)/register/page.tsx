  ...
  const handleSignIn = async () => {
    ...
    try {
      const { error: signInError } = await signIn.email({
        ...
        callbackURL: routes.default, // change to your dashboard or home page
        rememberMe: true,
      });
      ...
      setError(
        err instanceof Error
          ? err.message
          : t("general.login_error") || "Invalid credentials",
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-y-6 px-6 w-sm tracking-[-1.5px]">
        ...
        <FieldGroup
          className={cn("w-full gap-y-1.5", show_password_input && "gap-y-3.5")}
        >
          {/* —— EMAIL ADDRESS INPUT —— */}
          ...
          <Field orientation="horizontal">
            <Button
              ...
              className={cn("w-full mt-2.5", show_password_input && "mt-5")}
            >
              {show_password_input ? t("general.login_to_panel") : "continue —"}
            </Button>
          </Field>
        </FieldGroup>
      </div>
